...
const getTotal = async () => {
  ...
  try {
    ...
    return resData(STATUS.E300.result, STATUS.E300.resultDesc, moment().format('LT')); //moment().format 오후 2:43 와같은식으로 출력
  }
};
...
const todoController = {
  ...
  //reset  테이블을 초기화하고 postman으로 받은 len만큼 테이블에 더미데이터 삽입
  reset: async (req) => {
    ...
    try{
      const truquery = `TRUNCATE TABLE ${TABLE.TODO}`;
      const query = `INSERT INTO todo (title, done) VALUES ?`;
      const values = [[title, done]];
      await db.execute(truquery);
      console.log('dddd')
